...
8. **The graph model does not outperform top tabular or signature baselines on accuracy, and two independent stricter audits confirm that boundary.** On the binary NK-state task the GNN is statistically indistinguishable from LightGBM/XGBoost and from an SST-module signature baseline (§3.4). In the label-masked STAD→LIHC audit, learned multi-view fusion was significantly worse than no graph for AUROC and AUPRC and no individual view passed the joint stability/ablation gate (§3.7); the better unmasked result is a sensitivity analysis of label overlap (the NK-state label is partly defined by genes also present in the input features, item 15 below), not evidence of graph value. Separately, an exploratory single-edge cross-cohort transfer test (the `metabolic_crosstalk` edge, STAD→LIHC MCC) proved highly sensitive to training-seed count -- 3 seeds showed no detectable difference, 10 seeds showed the edge significantly *improving* transfer, the opposite of what a naive single-seed run had suggested -- so we do not report a directional claim from that test either. We therefore position typed views and edges as an auditable representation of authored structure, not as a predictor; the node-label-permutation calibration (§3.7) verifies where structure is imposed but cannot validate the underlying biology, and neither downstream-performance audit is stable enough to substitute for it.
...
- **Additional mechanisms.** The mechanism-card registry (`configs/mechanism_cards/registry.yaml`) is designed to hold multiple cards. Beyond the axis analyzed here, the registry already carries authored (not run end-to-end) cards for adenosine-mediated NK suppression, stress-ligand shedding (MICA/B-ADAM17), two checkpoint axes (CD155/CD112-TIGIT [competing with CD226] and HLA-E-NKG2A [competing with NKG2C]) motivated by the same NK-checkpoint biology already cited above [47,48], and two metabolite-mediated axes (kynurenine-AHR, lactate-NFAT) -- demonstrating the format across ligand-receptor, shedding, checkpoint-competition, and metabolite-crosstalk mechanism types. The TGFβ-driven NK exclusion card (§4.2) is the one other card run end-to-end. The CLEC12B-lipoprotein-lipase axis recently shown to restrain tumor NK cells and to synergize with PD-1 blockade [46] remains a natural next addition not yet authored.

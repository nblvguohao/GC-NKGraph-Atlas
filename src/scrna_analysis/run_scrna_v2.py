...
def main():
    ...
    for col in ["n_genes_by_counts", "total_counts"]:
        med = np.median(adata.obs[col])
        mad = median_abs_deviation(adata.obs[col])
        adata.obs[f"pass_{col}"] = (adata.obs[col] > med - 5*mad) & (adata.obs[col] < med + 5*mad)
    adata.obs["pass_mt"] = adata.obs["pct_counts_mt"] < 20
    adata = adata[adata.obs[["pass_n_genes_by_counts","pass_total_counts","pass_mt"]].all(axis=1)].copy()
    ...
    log(f"  HVG: {adata.var.highly_variable.sum()}")
